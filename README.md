...
企业入职 AI 助手 AgentOps 测评运营平台。当前仓库完成第 2 阶段业务底座：静态前端工程、响应式后台布局，以及虚构员工、知识、Skill、Tool 和业务目录的本地持久化管理。完整 Agent、Planner、Executor、Eval 与报告仍留待后续阶段。

> 本平台用于产品设计与 AI 测评演示。全部企业、员工、制度和运行指标均为虚构数据，不代表任何真实公司生产信息。

## 第 2 阶段页面

- `/#/catalog`：查看和筛选虚构员工、材料、账号、培训、办公与工单数据；
- `/#/knowledge`：编辑/启停知识并测试确定性的本地规则检索；
- `/#/skills`：编辑、启停、测试、查看 diff 与回滚 Skill 版本；
- `/#/tools`：启停并以 JSON 输入测试真实 IndexedDB 本地 Tool。

这些页面只提供能力注册中心和业务 Fixture 管理；完整 Agent、Planner、Executor 与 Eval 链路尚未实现。
...
应用首次启动会将第 2 阶段固定虚构 Fixture 导入 IndexedDB。打开“数据管理”可以：
